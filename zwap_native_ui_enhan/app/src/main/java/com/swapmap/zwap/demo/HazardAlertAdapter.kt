...
class HazardAlertAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // Set name (formatted nicely from type + confidence)
        holder.nameView.text = formatHazardName(hazard.name, hazard.type)
    ...
    companion object {
        /** Formats raw hazard names like "TRAFFIC_CALMING (85.0% conf, 3 reports)" into
         *  "Traffic calming · 85% conf · 3 reports" */
        fun formatHazardName(raw: String, type: FeatureType): String {
            // Pretty type label
            val prettyType = type.name
                .replace("_", " ")
                .lowercase()
                .replaceFirstChar { it.uppercase() }

            // If raw contains extra info in parens, extract and format it
            val parenMatch = Regex("\\((.+)\\)").find(raw)
            return if (parenMatch != null) {
                val details = parenMatch.groupValues[1]
                    .replace(".0%", "%")
                    .split(",")
                    .joinToString(" · ") { it.trim() }
                "$prettyType · $details"
            } else if (raw.isBlank() || raw == type.name) {
                prettyType
            } else {
                // If raw is already human-readable, titlecase it
                raw.replace("_", " ")
                    .lowercase()
                    .replaceFirstChar { it.uppercase() }
            }
